...
> **Include this in your project's agent configuration** by adding a reference to
> `references/github-issue-workflow.md` in your setup instructions.
...
Each task must be completable in a single agent session without running out of context.
...
- Created at `il-2-implement start`
...
| Command | Action |
|---------|--------|
| `il-setup` | Create labels, templates |
| `il-list` | List open issues |
| `il-1-plan N` | Load → scope → plan → approve (guided flow) |
| `il-1-plan N --quick` | Load issue, skip to status check |
| `il-2-implement` | Execute next failing task |
| `il-2-implement start` | Create branch, start from US-001 |
| `il-2-implement loop` | Auto-continue until blocked |
| `il-3-close` | Generate final report, create PR, archive after merge |
...
3. Run `il-2-implement` to retry with new context
...
### Completion
- User confirms "Works" → `debugState.status = "verified"` → Run `il-3-close`
