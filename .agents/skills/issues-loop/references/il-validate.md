...
## Usage
```
il-validate           # Run all validation checks
```
...
Configuration:
  ❌ .issueloop.config.json not found
     Run: il-setup to create configuration
...
============================
❌ 3 issues found. Run il-setup to fix label issues.
```
...
| Config missing | Not initialized | Run `il-setup` |
| Config invalid | Bad JSON | Check syntax in config file |
| Labels missing | Not created | Run `il-setup` |
