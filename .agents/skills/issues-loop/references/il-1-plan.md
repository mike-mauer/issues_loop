...
## Usage
```
il-1-plan 42             # Load and evaluate issue
il-1-plan #42            # Same, with # prefix
il-1-plan 42 --quick     # Skip scoping, load as-is
```
...
- **bugfix** → `references/formulas/bugfix.md` (reproduce → fix → verify)
- **feature** → `references/formulas/feature.md` (schema → logic → UI → integration)
- **refactor** → `references/formulas/refactor.md` (analyze → extract → migrate → verify)
...
- **Phase 2** uses the formula template from `references/formulas/{formula}.md` to guide task decomposition
...
6. **Apply the detected formula** - Use `references/formulas/{formula}.md` to guide task topology
7. **Persist formula** - Set `prd.json.formula` to the selected formula type (bugfix, feature, or refactor)

#### Task Format Reference

Each task must include (see `references/planning-guide.md` for full details):
...
- Resume from the appropriate phase (see `references/custom-planning-protocol.md`)
...
| Has prd.json, no task logs | Ready to Implement | Suggest `il-2-implement` |
| Has prd.json + partial task logs | In Progress | Show progress, suggest `il-2-implement` |
| All tasks passing | Ready for Testing | Suggest `il-2-implement` for testing checkpoint |
...
If branch exists, offer to check it out. If not, note that `il-2-implement` will create it.
...
Run il-2-implement to continue with US-002
```
