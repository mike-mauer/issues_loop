...
This protocol defines a custom 5-phase planning system that integrates seamlessly with the GitHub Issue Workflow. It replaces native plan mode with explicit phases, user-facing checkpoints, and full visibility into the planning process.
...
This protocol provides explicit phases, user-prompted checkpoints, and full visibility into the planning process.
...
├─ Load formula template from references/formulas/
...
2. **Detect formula type** - Classify the issue as bugfix, feature, or refactor:
   ...
   - If no label match, scan issue title + body for keywords (see `references/il-1-plan.md` Step 2b for full decision tree)
   - Default to `feature` when ambiguous
   - Load the matching formula template from `references/formulas/{formula}.md` — this guides task topology in Phase 2
...
- **Template:** `references/formulas/{formula}.md`
...
1. **Decompose** the requirement into discrete tasks following the formula template's task topology (e.g., bugfix: reproduce → fix → verify). Reference `references/formulas/{formula}.md` for default phases, acceptance criteria patterns, and verify command patterns.
...
Follow the format in `references/planning-guide.md`:
...
2. **Generate prd.json** following the schema in `references/planning-guide.md`
...
Options:
  1. "Yes, run il-2-implement" - Start implementation loop now
  2. "Not yet" - I'll start later
```

**On "Yes":** Suggest running `il-2-implement` or `il-2-implement start`
**On "Not yet":** End planning, user can return with `il-1-plan N --quick`
...
3. **Determine current phase:**
   ...
   - prd.json exists → Planning complete, suggest `il-2-implement`

---

## Integration with il-1-plan Command

The `il-1-plan` command's Step 4 (Planning Phase) should:

1. Reference this protocol: "Follow `references/custom-planning-protocol.md`"
2. Execute phases in order
3. Prompt the user for input at all checkpoints
4. NOT use native plan mode directly
