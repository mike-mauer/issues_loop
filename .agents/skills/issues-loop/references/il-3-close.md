...
## Usage
```
il-3-close              # Generate report, create PR, and auto-archive if PR merged
il-3-close --force      # Bypass verification requirement
il-3-close preview      # Preview report without posting
```

**Smart Behavior:** This command automatically detects the PR state:
- **No PR exists:** Generates report and creates PR
- **PR exists but not merged:** Shows PR status and next steps
- **PR merged:** Automatically archives and closes the issue
...
if [ "$DEBUG_STATUS" != "verified" ] && [ "$FORCE" != "true" ]; then
  ...
  echo ""
  echo "Options:"
  echo "  1. Run il-2-implement to go through the testing checkpoint"
  echo "  2. Use il-3-close --force to bypass (not recommended)"
  exit 1
fi
```
...
Before proceeding, check if a PR already exists and its merge status:

```bash
# Get issue number from prd.json
...
if [ -n "$PR_MERGED_AT" ] && [ "$PR_MERGED_AT" != "null" ]; then
  ...
  echo "Proceeding to archive..."
  # Jump to archive flow (Step 8)
  SKIP_TO_ARCHIVE=true
elif [ -n "$PR_NUMBER" ]; then
  echo "📋 PR #$PR_NUMBER exists (state: $PR_STATE)"
  echo ""
  echo "Next steps:"
  echo "  1. Review the PR in GitHub"
  echo "  2. Merge when approved"
  echo "  3. Run il-3-close again to archive"
  exit 0
fi
```

If PR is merged, the command skips directly to the archive step. If PR exists but not merged, it shows status and exits. Otherwise, it continues with report generation.
...
---
*Generated by the issues-loop skill from prd.json*"
```
...
## Archive Flow (Auto-triggered when PR merged)

When the PR has been merged (detected automatically in Step 1), the command proceeds directly to archiving:

### Step 9: Archive Implementation Record
...
ℹ️  This is a preview - nothing posted yet.
    Run il-3-close to post and create PR.
```
...
| Error | Response |
|-------|----------|
| Testing not verified | "Testing not complete. Run il-2-implement or use --force" |
...
Next steps:
  ...
  3. Merge when ready
  4. Run il-3-close again to auto-archive
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
