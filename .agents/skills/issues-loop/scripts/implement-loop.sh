...
PRD_FILE="prd.json"
LOG_FILE=".implement-loop.log"
LOCK_FILE=".implement-loop.lock"
MAX_ITERATIONS=${1:-20}
ITERATION=0
...
if [ ! -f "$PRD_FILE" ]; then
  log "$ICON_FAILURE prd.json not found. Run il-1-plan N first to create a plan."
  exit 1
fi
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if [ "$REMAINING" -eq 0 ]; then
    ...
    log "$LINE_HEAVY"
    log ""
    log "Run the agent and use il-2-implement to enter testing checkpoint."
  ...
  log "Gathering context for the agent..."
  ...
  # Build prompt for the agent with gathered context
...
CRITICAL: You MUST output exactly one of <result>PASS</result>, <result>RETRY</result>, or <result>BLOCKED</result> as the final line of your response."
  ...
  log "Running agent on $NEXT_TASK..."
  ...
  if [ $CLAUDE_EXIT -ne 0 ] && [ -z "$OUTPUT" ]; then
    log "$ICON_FAILURE Agent failed to execute (exit code: $CLAUDE_EXIT)"
    log "   Check that the agent CLI is installed and configured."
    exit 1
  fi

  # Log output to file (truncate if very long)
  echo "--- Agent output for $NEXT_TASK ---" >> "$LOG_FILE"
  ...
  if [ "$RESULT" = "PASS" ]; then
    ...
    log "$LINE_HEAVY"
    log ""
    log "Human input needed. Add guidance to the issue, then run il-2-implement."
